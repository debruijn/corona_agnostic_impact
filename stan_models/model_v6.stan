...
parameters {
    vector<lower=0>[W-1] week_eff_shock;
    vector[C] base_covid_eff;
    real<lower=0> sigma;
    real<lower=0> year_eff_0;
    vector<lower=0>[Y-1] year_eff_shock;
...
transformed parameters {
    ...
    covid_eff[1] = 0.0;
    for (c in 2:C+1) {
        covid_eff[c] = covid_eff[c-1] + base_covid_eff[c-1];
    }
...
model {
    ...
    week_eff_shock ~ normal(1, sigma_week);
    1.0 ~ normal(week_eff[W], sigma_week*8.25/7);
    year_eff_0 ~ cauchy(0, 1);
    year_eff_shock ~ normal(1, sigma_year);
